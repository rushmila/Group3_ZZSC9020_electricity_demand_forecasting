...
weather\sloppy{\cite{energygovau_2024_renewables}}. Electricity
...
to weather, the use of heating and cooling appliances, or many other
societal factors, such as visiting the beach or going shopping instead
of staying at home. Temperature is a crucial factor, as it directly
...
\item
  ...
  grid reliability.
\item
  MTLF expands the time scale from a few days to months ahead during a
  year. It helps with smart grid systems' maintenance, adequacy
  assessment, and fuel supply. Further, it plays a vital role in
  evaluating the financial attributes of energy systems by contributing
  to risk management.
\item
  LTLF forecasting involves a time scale ranging from months to even
  years. LTLF has been very important for production and load growth
  planning operations for a long time. Its significant advantage is that
  it can remove the effects of random fluctuations that occur in the
  short term and predict long-term trends.
\end{itemize}
...
As per \cite{9812604}, non-parametric models like Random Forests, SVR,
k-nearest Neighbors, and ANNs can capture complex, non-linear
relationships but are computationally intensive and more challenging to
interpret. Machine learning techniques, including ANNs, have
successfully forecasted load by modelling non-linear interactions
between load and weather variables.
...
accuracy and computational feasibility \cite{Suo}.
...
NSW air temperature (as measured from the Bankstown Airport weather
...
data). The literature review notes that temperature is a key driver of
demand. Therefore, this dataset is critically essential for the research
question
...
Use NSW Calendar for the model build to include holidays, seasons, and
weekend information.
...
All data files (excluding NSW Calendar) were stored in zip files.
Instead of exacting the the content of the files, we read directly the
zip file content for data cleaning. The only exception was the forecast
demand dataset, split into two zip files. Therefore, these files had to
be merged into one zip before consumption.
...
on the literature review, EDA and experimentation with several
univariate and multi-variate models we have considered the following
